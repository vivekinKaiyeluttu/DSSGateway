...
export class DocumentService {
  ...
  protected convertDateFromClient(document: IDocument): IDocument {
    const copy: IDocument = Object.assign({}, document, {
      expireAt: document.expireAt && document.expireAt.isValid() ? document.expireAt.format(DATE_FORMAT) : undefined,
      createdAt: document.createdAt && document.createdAt.isValid() ? document.createdAt.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((document: IDocument) => {
        document.expireAt = document.expireAt ? moment(document.expireAt) : undefined;
        document.createdAt = document.createdAt ? moment(document.createdAt) : undefined;
      });
    }
    return res;
  }
